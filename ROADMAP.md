...
- [ ] Ejemplo ejecutable para mobile (Flutter) — reintentado tres veces. Los intentos 1 y 2 (descarga directa del zip del SDK desde `storage.googleapis.com`) midieron ~100-120 KB/s pese a que la red de la sesión mejoró notablemente para otros hosts (python.org, pypi: hasta 24 MB/s). El intento 3 evitó ese host descargando el árbol fuente vía `git clone` de GitHub (~22 s, éxito total, requiere `git config --global core.longpaths true` por rutas largas de Windows bajo `dev/`), pero el primer arranque de `flutter --version` necesita bajar el binario del Dart SDK, que también se sirve desde `storage.googleapis.com` — quedó estancado en 4 KB durante más de 12 minutos sin avanzar. Conclusión: el bloqueo es específico de ese host/CDN para cualquier artefacto binario de Flutter/Dart, no del SDK completo ni de la sesión en general. Retomar solo si `storage.googleapis.com` responde, o si aparece un espejo/CDN alternativo para los binarios de Dart.
...
Siete de las ocho extensiones tienen ahora ejemplo ejecutable (typescript-node, angular, react, java-spring, dotnet, infrastructure, python). El patrón "extensión documentada → proyecto ejecutable que la implementa" queda demostrado a través de cinco toolchains genuinamente distintos (npm/Node, Gradle/Java, SDK de .NET, Terraform, Python embebido). Solo mobile (Flutter) queda pendiente, bloqueado por velocidad de red, no por alcance.
