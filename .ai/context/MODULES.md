...
| automation | Validadores, health score, quality gates, perfiles por ambiente, resolución de contexto, motor de políticas de gobernanza y CLI de bootstrap | `scripts/` | CLI: código de salida 0/≠0; `health-score --json`/`resolve-profile --json`/`resolve-context --json`/`classify-change --json` emiten JSON | `scripts/tests/` (descubiertas por `node --test`) | Mantenedores |
